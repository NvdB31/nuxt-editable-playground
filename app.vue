...
const onEditorRequestData = async (event) => {
...
const onEditorChangeData = async (event) => {
  ...
  switch (type) {
    ...
    case 'update':
      await $fetch(`/api/${payload.collection}/${payload.data.id}`, {
        method: 'PUT',
        body: payload.data
      })
      break
    ...
    case 'delete':
      if (payload.data.length) {
        await $fetch(`/api/${payload.collection}`, {
          method: 'DELETE',
          body: { ids: payload.data.map(i => i.id) }
        })
      } else {
        await $fetch(`/api/${payload.collection}/${payload.data.id}`, {
          method: 'DELETE'
